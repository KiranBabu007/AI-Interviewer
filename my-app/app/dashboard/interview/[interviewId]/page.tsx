...
// Define the shape of the interview details
...
interface Question {
  id: string;
  question: string;
  answerOptions?: string[]; // Optional for multiple-choice questions
...
const Page: React.FC = () => {
  const [interviewDetails, setInterviewDetails] = React.useState<InterviewDetails>(null);
  const [questions, setQuestions] = React.useState<Question[] | null>(null);
  const [activeQuestionIndex, setActiveQuestionIndex] = React.useState<number>(0);
  const { interviewid: interviewId } = useParams<{ interviewid: string }>();
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 h-full">
            ...
            <div className="bg-black/40 backdrop-blur-sm rounded-2xl border border-white/5 shadow-xl p-8 flex flex-col space-y-6">
              ...
              <div className="mt-auto flex justify-end gap-6">
                ...
                {activeQuestionIndex === questions?.length - 1 && interviewDetails && (
